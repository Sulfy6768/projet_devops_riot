...
class LolalyticsScraper:
    ...
    def _save_cache(self):
        ...
        try:
            ...
            data = {"timestamp": datetime.now().isoformat(), "matchups": self.cache}
    ...
    def fetch_champion_data(self, champion: str, role: str) -> Dict:
        ...
        try:
            ...
            with httpx.Client(timeout=15.0, follow_redirects=True) as client:
                ...
                weak_count = len(result.get("weak_against", []))
                print(f"Got {counters_count} counters, {weak_count} weak matchups for {champion}")
                return result
    ...
    def _parse_counter_data(self, data: Dict, champion: str) -> Dict:
        ...
        try:
            ...
            for matchup in counters_list:
                ...
                matchup_data = {"champion": champ_name, "winrate": round(vs_wr, 2), "games": games}
            ...
            # Trier par winrate
            result["counters"].sort(
                key=lambda x: x["winrate"]
            )  # Les plus bas en premier (nos pires matchups)
            result["weak_against"].sort(
                key=lambda x: x["winrate"], reverse=True
            )  # Les plus hauts en premier
